...
public:
    ...
    vector<int> closestRoom(vector<vector<int>>& rooms, vector<vector<int>>& queries) {
        ...
        Node root(rooms[0][0], rooms[0][1]);
        for (int i = 1; i < rooms.size(); i++) {
            insert(&root, rooms[i][0], rooms[i][1]);
        }
        vector<int> result;
        for (auto q : queries) {
            int floor = findFloor(&root, q[0], q[1]);
            int ceil = findCeil(&root, q[0], q[1]);
            result.push_back(chooseBest(q[0], floor, ceil));
        }
        return result;
    }

    Node* insert(Node* parent, int value, int size) {
        if (parent == nullptr) {
        ...
        if (value < parent->value) {
            ...
            parent->left = insert(parent->left, value, size);
        } else {
            parent->max_size_right = max(parent->max_size_right, size);
            parent->right = insert(parent->right, value, size);
        }
        return parent;
    }

    int findFloor(Node* parent, int value, int min_size) {
        if (parent == nullptr) {
            return -1;
        }
    ...
    int findCeil(Node* parent, int value, int min_size) {
        if (parent == nullptr) {
            return -1;
        }
        if (parent->size >= min_size && parent->value == value) {
